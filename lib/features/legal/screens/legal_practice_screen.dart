...
class _LegalPracticeScreenState extends State<LegalPracticeScreen> {
  ...
  Widget _dashboard(){ final d=Map<String,dynamic>.from(_data??{}); final cards=<String,String>{'Open matters':'openMatters','Open intakes':'openIntakes','Pending conflicts':'pendingConflicts','Hearings – next 30 days':'upcomingHearings','Documents awaiting completion':'draftDocuments','Trust reconciliations open':'unreconciledTrust'}; return ListView(padding:const EdgeInsets.all(20),children:[Wrap(spacing:16,runSpacing:16,children:cards.entries.map((e)=>SizedBox(width:250,child:Card(child:Padding(padding:const EdgeInsets.all(20),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(e.key),const SizedBox(height:8),Text('${d[e.value]??0}',style:Theme.of(context).textTheme.headlineMedium)])))).toList()),const SizedBox(height:24),Card(child:ListTile(leading:const Icon(Icons.folder_open),title:const Text('Matter register'),subtitle:const Text('Open and manage all legal matters'),trailing:const Icon(Icons.chevron_right),onTap:()=>context.go('/cases')))]); }
