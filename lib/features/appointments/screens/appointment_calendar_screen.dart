...
class _AppointmentBookingDialogState extends State<AppointmentBookingDialog> {
  ...
  Future<void> _quickCreateCustomer() async {
    final customer = await showDialog<Partner>(
      context: context,
      barrierDismissible: false,
      builder: (context) => const _QuickCustomerCreateDialog(),
    );
                ...
                children: [
                  ...
                  if (!_isEditing) ...[
                    PartnerSearchDropdown(
                      key: ValueKey(_customer?.id ?? 'appointment-customer-search'),
                      role: 'CUSTOMER',
                      label: 'Search customer',
                      initialPartner: _customer,
                      onPartnerSelected: (partner) => setState(() => _customer = partner),
                      validator: (_) => _customer == null ? 'Customer is required' : null,
                    ),
                    Align(
                      alignment: Alignment.centerRight,
                      child: TextButton.icon(
                        onPressed: _isSaving ? null : _quickCreateCustomer,
...
class _QuickCustomerCreateDialog extends StatefulWidget {
  const _QuickCustomerCreateDialog();

  @override
  State<_QuickCustomerCreateDialog> createState() => _QuickCustomerCreateDialogState();
}

class _QuickCustomerCreateDialogState extends State<_QuickCustomerCreateDialog> {
  final _formKey = GlobalKey<FormState>();
