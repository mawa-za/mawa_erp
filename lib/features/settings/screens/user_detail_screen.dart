...
class _UserDetailScreenState extends State<UserDetailScreen> {
  ...
  Future<void> _editPolicy() async {
    ...
    String account=u.accountType,status=u.status,timeZone=u.timeZone;bool test=u.testUser,blocked=u.externalTransactionsBlocked,mfa=u.mfaRequired;
    final save=await showDialog<bool>(context:context,builder:(dialogContext)=>StatefulBuilder(builder:(context,setLocal)=>AlertDialog(title:Text('Access policy — ${u.username}'),content:SizedBox(width:650,child:SingleChildScrollView(child:Column(mainAxisSize:MainAxisSize.min,children:[
      ...
      SearchableDropdownFormField<String>(value:timeZone,decoration:const InputDecoration(labelText:'Display time zone'),items:const {'Africa/Harare':'CAT (UTC+02:00)','Africa/Johannesburg':'South Africa (UTC+02:00)','Africa/Gaborone':'Botswana (UTC+02:00)','Africa/Maputo':'Mozambique (UTC+02:00)','UTC':'UTC'}.entries.map((e)=>DropdownMenuItem(value:e.key,child:Text(e.value))).toList(),onChanged:(v)=>setLocal(()=>timeZone=v??'Africa/Harare')),const SizedBox(height:8),
    ...
    if(save!=true)return;
    if(!RegExp(r'^\d{10}$').hasMatch(cellphone.text.trim())){_showError('Contact Number must be 10 numeric digits');return;}
    try{await _users.updateUser(widget.userId,{'email':email.text.trim(),'cellphone':cellphone.text.trim(),'timeZone':timeZone,'userType':u.type,'status':status,'accountType':account,'testUser':test,'environmentScope':env.text.trim(),'externalTransactionsBlocked':blocked,'expiresAt':expiry.text.trim().isEmpty?null:'${expiry.text.trim()}T23:59:59','protectedReason':reason.text.trim(),'mfaRequired':mfa});final prefs=await SharedPreferences.getInstance();if(prefs.getString('userId')==widget.userId){await prefs.setString('timeZone',timeZone);AppDateUtils.configureTimeZone(timeZone);}await _load();}catch(e){_showError(e);}
  }
